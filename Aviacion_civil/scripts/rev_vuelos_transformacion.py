# ESCUDERO rev_FINAL ejercio 1 transformación funcionando V-01-01-2025
...
df_2021 = spark.read.option("header", "true").option("sep", ";").csv("hdfs://172.17.0.2:9000/ingest3/2021-informe-ministerio.csv")
df_2022 = spark.read.option("header", "true").option("sep", ";").csv("hdfs://172.17.0.2:9000/ingest3/202206-informe-ministerio.csv")
df_aeroport = spark.read.option("header", "true").option("sep", ";").csv("hdfs://172.17.0.2:9000/ingest3/aeropuertos_detalle.csv")
...
# Reemplazar "Doméstico" con "Domestico" en la columna 'Clasificación Vuelo'
df_2022 = df_2022.replace("Doméstico", "Domestico", subset=["Clasificación Vuelo"])
...
## OJO SI VAMOS A REESCRIBIR DATOS USAR ESTA OPCION vuelos_filtered_renamed.write.mode("overwrite").insertInto("vuelosdb.aeropuerto_tabla")
